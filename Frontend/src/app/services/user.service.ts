...
export class UserService {
  private baseUrl = 'http://localhost:7777/user';

  private followUrl='http://localhost:8888/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8888/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8888/follow/follower/`+`${userid}`)
  }
